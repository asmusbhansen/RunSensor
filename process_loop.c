...
short yz_angle_f = 0;
 ...
 void process_loop(mpu9250_sensor_values sensor_values, MPU9250_orientation * mpu_orientation){
      ...
      short xz_angle_acc_f = cATAN2(sensor_values.accl_Z, sensor_values.accl_Y, 10);
      short yz_angle_acc_f = cATAN2(sensor_values.accl_Z, sensor_values.accl_X, 10);
      //short xz_angle_acc_f_1 = (int32_t)(atan2((float)sensor_values.accl_X,(float)sensor_values.accl_Z) * 10431);

      //Convert gyro rate to
      short gyro_Y_f = ((-1) * sensor_values.gyro_Y * 32768) / (33 * 180 * 200);
      short gyro_X_f = ((-1) * sensor_values.gyro_X * 32768) / (33 * 180 * 200);
