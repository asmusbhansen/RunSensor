...
extern int compFilt(short, short, short);
 ...
 void process_loop_float(mpu9250_sensor_values sensor_values, MPU9250_orientation * mpu_orientation){
      //Complementary filter
          ...
          float gyro_Y = (float)(-1*sensor_values.gyro_Y) / GYRO_SCALE;
          float gyro_X = (float)(-1*sensor_values.gyro_X) / GYRO_SCALE;

          float xz_angle = filter_constant*(xz_angle + gyro_Y * UPDATE_LOOP_DT/1000 ) + (1-filter_constant)*xz_angle_acc;
          float yz_angle = filter_constant*(yz_angle + gyro_X * UPDATE_LOOP_DT/1000 ) + (1-filter_constant)*yz_angle_acc;
      ...
      if (counter >= 100) {
        ...
        NRF_LOG_INFO("Angle is XZ %d", xz_angle);
